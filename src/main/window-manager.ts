...
class WindowManager {
  private readonly actives: Array<{
    win: BrowserWindow;
    events: Set<string>;
  }> = [];
  ...
  public create(
    ...
    if (!options.title) {
      options.title = `${nameOfNetwork(config.genesis.id).toUpperCase()}:${
        config.name
      }`;
    }
    console.log(options)
    ...
    if (entry) {
      events.forEach(event => {
        if (!entry.events.has(event)) {
          ...
          entry.win.on(event as any, () =>
            entry.win.webContents.send("browser-window-event", event)
          );
        }
      });
    }
  ...
  public dispatchDbEvent(event: DbEvent) {
    this.actives.forEach(entry =>
      entry.win.webContents.send("db-event", event)
    );
  }
  public focus() {
  ...
  public openUrl(externalUrl: string) {
    ...
    const config = presets.find(
      p => nameOfNetwork(p.genesis.id) === parsed.network
    );
    if (config) {
      for (const entry of this.actives) {
        const win = entry.win;
        if (
          win.webContents.getWebPreferences().nodeConfig!.genesis.id ===
          config.genesis.id
        ) {
          target = win;
          break;
