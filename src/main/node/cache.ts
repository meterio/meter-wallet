...
export class Cache {
  ...
  public async getBlock(
    ...
    const slot =
      typeof rev === "string"
        ? this.slots.get(rev)
        : this.window.find(s => s.number === rev);
    if (slot && slot.block) {
      return slot.block;
    }
    ...
    }
    {
      ...
      if (tx) {
        ...
        if (
          this.window.length > 0 &&
          tx.meta.blockNumber <
            this.window[0].number + this.window.length - WINDOW_LEN
        ) {
  ...
  public async filter<T extends "event" | "transfer">(
    ...
    FETCH: for (let i = this.window.length - 1; i >= 0; i--) {
      ...
      if (filter) {
        for (let j = i + 1; j < this.window.length; j++) {
          ...
          if (bloom) {
            if (
              filter.bloomKeys.length === 0 ||
              filter.bloomKeys.some(k => testBytesHex(bloom, k))
            ) {
              break FETCH;
            }
          } else {
    ...
    const headSlot =
      this.window.length > 0 ? this.window[this.window.length - 1] : undefined;
