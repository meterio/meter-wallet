...
export function createAccountVisitor(
  ...
  return {
    ...
    get: () => {
      return client.getAccount(addr, client.head.id).then(cloneDeep);
    },
    getCode: () => {
      return client.getCode(addr, client.head.id).then(cloneDeep);
    },
    getStorage: key => {
      ensure(V.isBytes32(key), `'key' expected bytes32 in hex string`);
      return client.getStorage(addr, key, client.head.id).then(cloneDeep);
    },
    method: jsonABI => {
