    ...
    const handleEvent = (ev: Event) => {
      if (ev.type === "new-window") {
        ...
        if (normalNavigate && this.errorCode === 0) {
          AccessRecords.record(this.webview.src!, this.title, this.favicon);
        }
        this.progress = 1;
        ...
        this.favicon =
          favicons.find(i => i.includes("32x32")) || favicons[0] || "";
        ...
        if (loadCommit.isMainFrame) {
          if (loadCommit.url !== this.currentHref) {
            if (
              ...
              NodeUrl.parse(loadCommit.url).host
            ) {
              this.title = "";
              this.favicon = "";
              this.cert = null;
        ...
        if (didFailLoad.isMainFrame && didFailLoad.errorCode !== -3) {
          ...
          const obj = errorMap.get(didFailLoad.errorCode) || {
            name: "",
            desc: ""
          };
