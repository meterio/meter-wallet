  ...
  >
    <v-card>
      <v-layout row style="height:500px;">
        <v-layout column style="width:400px;flex:0 0 auto;background-color:rgba(0,0,0,0.1);" pa-1>
          ...
          <textarea :value="arg.message.payload.content" readonly class="payload pa-2 white serif" />
        </v-layout>
        <v-layout column style="width:300px">
          <v-card-text>
            <WalletSeeker :wallets="arg.wallets" v-model="arg.selectedWallet" :disabled="signing" />
          </v-card-text>
          <v-card-text>
            <Tip
...
import { Certificate, cry } from "meter-devkit";
...
export default class CertSigningDialog extends Mixins(
  class extends DialogHelper<Arg, Connex.Vendor.SigningService.CertResponse> {}
) {
  opened = false;
  password = "";
  passwordError = "";
  ...
  get readyToSign() {
    return (
      !this.signing && this.passwordRules.every(r => r(this.password) === true)
    );
  }
  get wallet() {
    return this.arg.wallets[this.arg.selectedWallet];
  }
  ...
  async sign() {
    ...
    try {
      ...
      const annex = {
        ...
        timestamp: connex.meter.status.head.timestamp,
      ...
      const signature =
        "0x" +
        cry.secp256k1
          .sign(cry.blake2b256(unsigned), privateKey)
          .toString("hex");
