  ...
  >
    <v-card class="bg">
      ...
      <v-card-actions class="signing-footer" style="flex: 0 0 auto">
        <v-btn :disabled="signing" small text @click="decline">Decline</v-btn>
        <v-spacer />
        <v-btn
          v-show="step === 2"
          small
...
export default class CertSigningDialog extends Mixins(
  class extends DialogHelper<Arg, Flex.Vendor.CertResponse> {}
) {
  opened = false;
  password = "";
  passwordError = "";
  ...
  get unsignedHexStr() {
    return this.unsignedHex
      ? this.unsignedHex.toUpperCase().slice(0, 4) +
          "…" +
          this.unsignedHex
            .toUpperCase()
            .slice(this.unsignedHex.toUpperCase().length - 4)
      : "";
  }
