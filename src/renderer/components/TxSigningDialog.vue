...
type Arg = {
  message: Flex.Driver.SignTxArg;
  wallets: walletList;
  selectedWallet: string;
  suggestedGas: number;
  txComment: string;
  dependsOn: string | null;
  delegationHandler?: Flex.Vendor.DelegationHandler;
};
...
@Component
export default class TxSigningDialog extends Mixins(
  class extends DialogHelper<Arg, Result> {},
  AccountLoader
) {
  opened = false;
  signing = false;
  ...
  get value() {
    return this.clauses.reduce((v: BigNumber, c: Flex.Meter.Clause) => {
  ...
  get readyToSign() {
    return !this.signing && this.estimation.gas;
  }
  ...
  async localSign() {
    ...
    try {
      ...
      const timestamp = flex.meter.status.head.timestamp;
      const signedTx = this.builtTx!.signTx(
        privateKey,
        this.delegation.signature
      );
      const result = {
