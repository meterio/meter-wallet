    ...
    <v-card>
      <v-layout row style="height:450px;">
        ...
        <v-layout column style="width:300px;">
          <v-card-text>
            <WalletSeeker :wallets="wallets" v-model="arg.selectedWallet" :disabled="signing" />
...
export default class TxSigningDialog extends Mixins(
  ...
  get wallets() {
    return this.arg.wallets;
  }
  get wallet() {
    return this.wallets[this.arg.selectedWallet];
  }
  get address() {
  ...
  async sign() {
    ...
    try {
      ...
      const timestamp = connex.meter.status.head.timestamp;
