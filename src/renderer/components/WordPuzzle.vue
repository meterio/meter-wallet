...
export default class WordPuzzle extends Vue {
  ...
  get shuffledWords() {
    return shuffle(
      this.safeWords.map<Word>(w => ({ value: w, picked: false }))
    );
  }
  ...
  verifyWord(word: Word) {
    return (
      this.pickedWords.indexOf(word) === this.safeWords.indexOf(word.value)
    );
  }

  get resolved() {
    return (
      this.pickedWords.length === this.safeWords.length &&
